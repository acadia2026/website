---
layout: post
title:  ""
date:   2014-06-01
caption:  Integration and Interoperability
permalink: /Integration-Interoperability
project: true
category: site-workshop
thumbnail: assets/images/workshops/acadia26-workshop_RhinoInside.png

---
<span style="font-size: 24px; font-weight: bold;">Integration and Interoperability:</span>
**Rhino and Revit Workflows with Rhino.Inside and Conveyor**

# Workshop Dates: 10/21
<strong>Location: <a href="https://maps.app.goo.gl/in1RRBHwCzTM35Ks7" target="_blank" rel="noopener noreferrer">Downtown, Detroit at Gensler</a></strong>
**Address:** 1265 Griswold St, Detroit, MI 48226

<p style="margin: 0; font-size: 15px; font-weight: bold;">Workshop Team</p>
<p style="margin: 0;"><strong> Nate Miller</strong>, Proving Ground, <a href="https://provingground.io/" target="_blank" rel="noopener noreferrer">instagram</a></p>
</p>

## Registration: Regular - $175, Student - $90
# <a href="https://www.eventbrite.com/e/acadia-2026-workshops-tickets-1994103898440?aff=oddtdtcreator" target="_blank" rel="noopener noreferrer">Registration</a>

---

## Workshop Description
Architects, engineers, and designers use a variety of digital tools to develop their architectural ideas and documentation. This workshop will explore practical, productive, and efficient integrations between Rhino and Revit to bridge design with BIM production. Participants will engage in hands-on demonstrations and examples for transferring geometry and data to support real-world design. Additionally, participants will explore novel automation opportunities supporting BIM quality assurance.  

Learning Objectives
•	Learn about the relationships between geometric types and BIM object classification.
•	Learn direct integrations for translating geometry and data between Rhino and Revit.
•	Learn model management techniques for tracking changes between modeling tools.
•	Learn computational automation techniques for Revit model manipulation.

## Workshop Schedule

### Day 1 Morning 
**Time**: 9:00 am – 1:00 pm 
Introduction to Rhino and Revit integrations with Conveyor

### Day 1 Afternoon
**Time**: 2:00 pm – 5:00 pm 
Workflow Customization with Rhino.Inside and Conveyor

**Total Workshop Time**: ~7 hours

## Participant Information
- **Audience**: Students, researchers, and professionals in architecture, design
- **Skill Level**: Intermediate. Prior knowledge of Rhino/Grasshopper is helpful but not required
- **Group Size**: ~20 participants
- **Hardware**: laptop capable of running Rhino 8 with Grasshopper

## Workshop Gallery

{% include image-gallery.html folder="assets/images/workshops/RhinoInside" item_width="30%" show_caption="false" %}

## Refund and Change Policy

**Important:** Refund and change requests will not be accepted beyond Monday, October 5th.
